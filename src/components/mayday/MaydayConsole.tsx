...
export function MaydayConsole() {
  ...
  const [realCallEnabled, setRealCallEnabled] = useState<boolean>(
    () => lsGet("mayday.real") === "1",
  );
  const [watchShop, setWatchShop] = useState<boolean>(() => lsGet("mayday.watch") === "1");
  const [remoteShopUrl, setRemoteShopUrl] = useState<string>(() =>
    lsGet("mayday.remote", "http://192.248.185.175"),
  );
  ...
  useEffect(() => {
    if (typeof window !== "undefined") localStorage.setItem("mayday.from", fromNumber);
  }, [fromNumber]);
  useEffect(() => {
    if (typeof window !== "undefined")
      localStorage.setItem("mayday.real", realCallEnabled ? "1" : "0");
  }, [realCallEnabled]);
  useEffect(() => {
    if (typeof window !== "undefined") localStorage.setItem("mayday.watch", watchShop ? "1" : "0");
  }, [watchShop]);
  useEffect(() => {
    if (typeof window !== "undefined") localStorage.setItem("mayday.remote", remoteShopUrl);
    remoteUrlRef.current = remoteShopUrl;
  }, [remoteShopUrl]);
  useEffect(() => {
    if (typeof window !== "undefined") localStorage.setItem("mayday.remoteToken", remoteShopToken);
    remoteTokenRef.current = remoteShopToken;
  }, [remoteShopToken]);
  useEffect(() => {
  ...
  useEffect(() => {
    incidentRefRef.current = incidentRef;
  }, [incidentRef]);
  ...
  const clearTimers = useCallback(() => {
    ...
    if (verifyRef.current) {
      window.clearInterval(verifyRef.current);
  ...
  useEffect(() => {
    ...
    const id = window.setTimeout(
      () => setBriefIndex((i) => Math.min(i + 2, PHONE_BRIEF.length)),
      28,
    );
  ...
  const breakShops = useCallback(async () => {
    ...
    fetch("/api/shop/admin/break", {
      ...
      body: JSON.stringify({
        on: true,
        reason: "INVENTORY_SERVICE_URL → dead port :9999 after deploy abc123",
      }),
    }).catch(() => {
    ...
    if (url) {
      fetch("/api/vultr-shop/break", {
        ...
        body: JSON.stringify({
          url,
          action: "break",
          token: remoteTokenRef.current.trim() || undefined,
        }),
      })
        .then(async (r) => {
          ...
          setRemoteStatus(
            j.ok
              ? `remote break OK · ${j.latency_ms}ms`
              : `remote break FAIL · ${j.error ?? j.upstream_status ?? "?"}`,
          );
        })
    ...
    if (url) {
      fetch("/api/vultr-shop/break", {
        ...
        .then(async (r) => {
          ...
          setRemoteStatus(
            j.ok
              ? `remote repair OK · ${j.latency_ms}ms`
              : `remote repair FAIL · ${j.error ?? j.upstream_status ?? "?"}`,
          );
        })
  ...
  const probeAll = useCallback(async (): Promise<{ green: boolean; probe: string }> => {
    ...
    try {
      ...
      probe = "internal probe failed";
    }
    const url = remoteUrlRef.current.trim();
    if (!url) return { green: internalGreen, probe: `${probe} · 60s clean window` };
    try {
      const r = await fetch(`/api/vultr-shop/health?url=${encodeURIComponent(url)}`, {
        cache: "no-store",
      });
  ...
  const startVerification = useCallback(() => {
    ...
    const tick = async () => {
      ...
      const pmId = window.setTimeout(
        () => setPostmortem(buildPostmortem({ incidentId: inc, durationSecs, euroLost: euro })),
        1500,
      );
  ...
  const breakProduction = useCallback(
    (source: "manual" | "watchdog", reason?: string) => {
      clearTimers();
      setEvents([]);
      setRinging(false);
      setPostmortem(null);
      ...
      if (choice === "wait") {
        ...
        setEvents((prev) => [
          ...prev,
          {
            ...
            body: "Service still down. MAYDAY re-asks in 20s.",
          },
        ]);
        // The incident is still burning money — ring the on-call again.
        const id = window.setTimeout(() => {
          if (phaseRef.current !== "awaiting_approval") return;
          setEvents((prev) => [
            ...prev,
            {
              id: `${Date.now()}`,
              ts: nowIso(),
              type: "calling",
              title: "☎ Re-asking on-call — hold elapsed, service still degraded",
              body: "Dialing again…",
            },
          ]);
  ...
  useEffect(() => {
    ...
    startCall({
      data: {
        ...
        from: fromNumber,
        brief: PHONE_BRIEF,
        stateUrl: remoteUrlRef.current.trim() || undefined,
      },
    })
      .then((r) => {
        setTwilioId(r.id);
    ...
    const id = window.setInterval(async () => {
      try {
        ...
        if (r.decision) {
          ...
          if (!callAnswered) {
            setCallAnswered(true);
            setBriefIndex(PHONE_BRIEF.length);
          }
          decide(r.decision);
        }
      } catch {
  ...
  useEffect(() => {
    ...
    const tick = async () => {
      ...
      if (remoteShopUrl.trim()) {
        try {
          const r = await fetch(
            `/api/vultr-shop/health?url=${encodeURIComponent(remoteShopUrl.trim())}`,
            { cache: "no-store" },
          );
          const h = await r.json();
  ...
  const remoteAction = useCallback(
    async (action: "break" | "repair") => {
      ...
      try {
        ...
        setRemoteStatus(
          j.ok
            ? `remote ${action} OK · ${j.latency_ms}ms`
            : `remote ${action} FAIL · ${j.error ?? j.upstream_status ?? "?"}`,
        );
      } catch (e) {
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      {showConfig && (
        <ConfigPanel
          ...
          remoteShopUrl={remoteShopUrl}
          setRemoteShopUrl={setRemoteShopUrl}
          remoteShopToken={remoteShopToken}
          setRemoteShopToken={setRemoteShopToken}
        ...
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-2 text-mono text-[10px] uppercase tracking-[0.25em] text-muted-foreground">
              ...
              {live && (
                <span className="bg-foreground px-1.5 py-px text-[9px] text-background">
                  LIVE CALL
                </span>
              )}
              {watching && (
        ...
        <div className="ml-auto flex flex-wrap items-center gap-2">
          <div className="flex items-stretch divide-x divide-border border border-border text-mono">
            <div className="px-3 py-1.5">
              ...
              <div
                className={`text-base font-bold leading-tight tabular-nums ${active && isBroken ? "text-danger" : ""}`}
              >
                {euroLost.toFixed(1)}
              </div>
            </div>
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <label className="flex items-center gap-2 text-mono text-xs">
      <span className="w-24 shrink-0 uppercase tracking-widest text-[10px] text-muted-foreground">
        {label}
      </span>
      {children}
    </label>
...
function ConfigPanel({
  ...
  remoteShopUrl: string;
  setRemoteShopUrl: (v: string) => void;
  remoteShopToken: string;
  setRemoteShopToken: (v: string) => void;
  remoteStatus: string;
  ...
  const input =
    "min-w-0 flex-1 border border-border bg-background px-2.5 py-1.5 text-mono text-xs outline-none focus:bg-muted";
  return (
    <div className="border-b border-border bg-muted/60">
      <div className="mx-auto grid max-w-[1440px] grid-cols-1 gap-x-8 gap-y-3 px-4 py-4 md:grid-cols-2 lg:px-6">
        <div className="space-y-3">
          ...
          {callStatus && (
            <div className="border border-border bg-background px-2.5 py-1.5 text-mono text-[11px]">
              {callStatus}
            </div>
          )}
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                className="border border-border p-1 hover:bg-foreground hover:text-background"
              >
          ...
          <Field label="VM URL">
            <input
              type="url"
              value={remoteShopUrl}
              onChange={(e) => setRemoteShopUrl(e.target.value.trim())}
          ...
          <Field label="Token">
            <input
              type="password"
              value={remoteShopToken}
              onChange={(e) => setRemoteShopToken(e.target.value)}
          ...
          <div className="flex items-center gap-2">
            ...
            >
              {remoteBusy === "break" ? "breaking…" : "Break VM shop"}
            </button>
            <button
              disabled={remoteBusy !== null || !remoteShopUrl.trim()}
            ...
            >
              {remoteBusy === "repair" ? "repairing…" : "Repair VM shop"}
            </button>
            {remoteStatus && (
              <span className="min-w-0 truncate text-mono text-[11px] text-muted-foreground">
                {remoteStatus}
              </span>
            )}
          </div>
  ...
  return (
    <section className="panel flex min-h-[520px] flex-col overflow-hidden lg:h-[calc(100vh-140px)]">
      <div className="flex items-center justify-between border-b border-border px-4 py-2.5">
        ...
        <span className="hidden text-mono text-[10px] uppercase tracking-widest text-muted-foreground sm:inline">
          Vultr Serverless Inference · LiteLLM
        </span>
      </div>
    ...
    <div className="grid h-full place-items-center text-center">
      <div className="max-w-md">
        ...
        <h2 className="mt-3 text-3xl font-bold leading-tight">
          Your infrastructure just called.
          <br />
          It fixed itself.
        </h2>
        <p className="mx-auto mt-4 max-w-sm text-sm text-muted-foreground">
          Hit <span className="text-mono font-bold text-danger">Break production</span> to push a
          real config regression to the shop. The agent investigates, cites its runbooks, phones the
          on-call human, and closes the loop.
        </p>
        <div className="mx-auto mt-6 grid max-w-sm grid-cols-3 border border-border text-mono text-[10px] uppercase tracking-widest">
          {["detect", "decide", "call", "approve", "fix", "verify"].map((s, i) => (
            <div
              key={s}
              className={`px-2 py-2 ${i % 3 !== 2 ? "border-r border-border" : ""} ${i < 3 ? "border-b border-border" : ""}`}
            >
              {s}
            </div>
          ))}
  ...
  return (
    <li>
      <div className="flex items-start gap-3">
        <div className="w-16 shrink-0 pt-0.5 text-mono text-[10px] tabular-nums text-muted-foreground">
          {e.ts}
        </div>
        <div className="min-w-0 flex-1">
          <div className="flex flex-wrap items-baseline gap-2">
            <span
              className={`text-mono text-[10px] font-bold uppercase tracking-widest ${typeColor(e.type)}`}
            >
              {typeLabel(e.type)}
            </span>
          ...
          {e.tool && (
            <div className="mt-1 border border-warning bg-background px-2 py-1.5 text-mono text-xs">
              ...
              <span className="text-muted-foreground">
                ({JSON.stringify(e.args ?? {}).slice(1, -1) || "…"})
              </span>
            </div>
          )}
          {e.result && (
    ...
    <section className="panel overflow-hidden">
      ...
      <div className="grid grid-cols-3 divide-x divide-border border-b border-border text-mono">
        <MetricCell
          ...
          tone={isBroken ? "danger" : "success"}
        />
        <MetricCell
          label="p95"
          value={`${metrics.p95_ms}ms`}
          tone={metrics.p95_ms > 500 ? "warning" : "muted"}
        />
      ...
      <div className="border-b border-border px-4 py-3">
        ...
        <div
          className={`flex h-20 items-end gap-[3px] border p-2 ${isBroken ? "border-danger" : "border-border"}`}
        >
          {bars.map((v, i) => (
            <div
              key={i}
      ...
      <div className="border-b border-border px-4 py-3">
        <div className="mb-1.5 text-mono text-[10px] uppercase tracking-widest text-muted-foreground">
          live probes
        </div>
        <div className="space-y-1 text-mono text-[11px]">
          <div className="flex items-center gap-2">
            <span
              className={`h-1.5 w-1.5 ${!watching ? "bg-border" : liveHealth?.green ? "bg-success" : "bg-danger"}`}
            />
            ...
            <span className="ml-auto tabular-nums">
              {!watching
                ? "watchdog off"
                : liveHealth
                  ? `err ${(liveHealth.error_rate * 100).toFixed(1)}% · ${liveHealth.orders} orders`
                  : "probing…"}
            </span>
          </div>
          <div className="flex items-center gap-2">
            <span
              className={`h-1.5 w-1.5 ${!watching ? "bg-border" : remoteOk ? "bg-success" : remoteDown ? "bg-danger" : "bg-warning"}`}
            />
            <span className="text-muted-foreground">vultr vm</span>
            <span className="ml-auto max-w-[60%] truncate tabular-nums">
              {!watching ? "—" : remoteStatus || "probing…"}
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="px-4 py-3">
        ...
        <ul className="space-y-1 text-mono text-[11px]">
          <li
            className={`flex items-center gap-2 ${isBroken && phase !== "resolved" ? "text-danger" : "text-muted-foreground"}`}
          >
      ...
      <div className="border-t border-border bg-muted px-4 py-2.5">
        <div className="flex flex-wrap gap-1 text-mono text-[10px]">
          {[
            ...
            <span
              key={t}
              className="border border-border bg-background px-1.5 py-0.5 text-muted-foreground"
            >
              {t}
            </span>
          ))}
    ...
    <section className="panel mt-4 overflow-hidden">
      ...
      >
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="min-w-0">
            <div className="text-mono text-[10px] uppercase tracking-widest text-success">
              04 · post-mortem · committed
            </div>
        ...
        <div className="hidden shrink-0 items-center gap-4 text-mono text-xs sm:flex">
          ...
          <span>
            <span className="text-muted-foreground">impact </span>€{euroLost.toFixed(0)}
          </span>
          <span>{open ? "▲" : "▼"}</span>
        </div>
      </button>
  ...
  return (
    <div className="space-y-3">
      {blocks.map((b, i) => {
        ...
        if (b.startsWith("## "))
          return (
            <h2 key={i} className="mt-4 border-b border-border pb-1 text-lg font-bold">
              {b.slice(3)}
            </h2>
          );
        if (b.startsWith("- ")) {
  ...
  return (
    <>
      {parts.map((p, i) => {
        ...
        if (p.t === "c")
          return (
            <code key={i} className="bg-muted px-1 py-0.5 text-mono text-[12px]">
              {p.v}
            </code>
          );
